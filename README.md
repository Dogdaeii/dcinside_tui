...
* 🎨 **16종의 다이내믹 커스텀 스킨 (Dynamic Themes)**
  * 'VS Code 다크', 'Claude Code', 'NASA 콘솔', '매트릭스' 등 감성 넘치는 16가지 해커 스타일 테마를 지원합니다.
...
* `t` : **스킨(테마) 선택 팝업 열기** (전역)
* `w` : **브라우저 선택 팝업 열기** (전역)
* `u` : **앱 자동 업데이트** (전역, GitHub에서 최신 버전 가져오기)
...
* **v1.2.1**
  * **[버그 수정]** 대댓글(답글)의 들여쓰기용 공백 4칸이 Markdown 코드 블록으로 해석되어, 창 크기 조절 시 자동 줄바꿈이 되지 않던 문제를 수정했습니다.
* **v1.2.0**
  * **[호환성 개선]** 윈도우(Windows) 및 리눅스 운영체제에서도 에러 없이 완벽하게 구동되도록 크로스 플랫폼 지원을 추가했습니다. (미디어 열기, 브라우저 연동 기능 정상화)
  * **[기능 개선]** 사무실에서 일하는 척하기 완벽한 스킨 2종('VS Code 다크', 'Claude Code')을 새롭게 추가하여 총 16개의 테마를 지원합니다.
  * **[기능 개선]** 앱 내부에서 최신 버전으로 바로 업데이트할 수 있는 자동 업데이트 단축키(`u`)를 새롭게 추가했습니다.
* **v1.1.1**
